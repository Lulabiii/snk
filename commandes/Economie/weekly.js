...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('argentʰᵉᵇᵈᵒᵐᵃᵈᵃⁱʳᵉ')
        .setDescription("Permets à l'utilisateur d'obtenir un petit montant d'argent chaque semaine."),
    ...
    async execute(interaction) {
        ...
        try {
            ...
            if (result.length > 0) {
                const { weekly, prévenu_weekly } = result[0];
        ...
        \`\`\` \`\`\`
                ...
                await db.query(
                    'INSERT INTO cooldown (id_membre, weekly, prévenu_weekly) VALUES (?, ?, 0)',
        ...
        > <:Sans_titre_349_20240518230508Cop:1304168153680707604> **[ <:gold:1304167372999098489> ] — __Argᥱᥒt__**
        <:Sans_titre_349_20240519142111Cop:1304168162392019066> **${interaction.user}, souhaitez réellement obtenir votre argent hebdomadaire ? Il vous faudra ensuite attendre 1 semaine afin de pouvoir l'obtenir à nouveau.**
        ...
        \`\`\` \`\`\`
            ...
            collector.on('collect', async (btnInteraction) => {
                if (btnInteraction.customId === 'button_oui') {
                    await db.query(
                        'UPDATE cooldown SET weekly = ?, prévenu_weekly = 0 WHERE id_membre = ?',
                        [expirationTime, memberId]
                    );
                    const argent_random = Math.floor(Math.random() * (10000 - 5000 + 1)) + 5000;
                    await db.execute(
...
setInterval(async () => {
    ...
    > <:Sans_titre_349_20240518230508Cop:1304168153680707604> **[ <:gold:1304167372999098489> ] — __Argᥱᥒt__**
    <:Sans_titre_349_20240519142111Cop:1304168162392019066> **${user}, le cooldown de votre obtention d'argent hebdomadaire vient de passer à l'instant, vous pouvez vous ravitailler à nouveau en utilisant la commande \`/argentʰᵉᵇᵈᵒᵐᵃᵈᵃⁱʳᵉ\`.**
    ...
    \`\`\` \`\`\`
                ...
                await db.query('UPDATE cooldown SET prévenu_weekly = 1 WHERE id_membre = ?', [id_membre]);
            }
        }
    } catch (error) {
